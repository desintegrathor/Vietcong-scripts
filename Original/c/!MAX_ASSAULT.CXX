...
dword gBombStatus[NUM_BOMBS]; // !!!!!!!!!!!!!
float gBombTimer[NUM_BOMBS];
float gAfterExplodeTimer[NUM_BOMBS];
...
float gUpdate_BombTimer[NUM_BOMBS];
float gRecv_BombTimer[NUM_BOMBS];

float gCLN_BombTimer[NUM_BOMBS];
dword gCLN_BombStatus[NUM_BOMBS];
...
BOOL SRV_CheckEndRule(float time){
	...
	switch(gEndRule){
		case SC_MP_ENDRULE_TIME:
			...
			for (i=0; i<NUM_BOMBS; i++) if (gBombStatus[i]!=BOMB_STATUS_STARTING) continue;
			if (i <= NUM_BOMBS) {	//nejaka neni starting
				gTime += time;
		...
		if (gBombStatus[i]==BOMB_STATUS_ACTIVE){
			...
			if (gBombTimer[i]<=0.0f){
				...
				for (k=0; k<NUM_BOMBS; k++)
					if ((SC_ggi(GVAR_BOMBSTATUS__+k*2) != BOMB_STATUS_EXPLODING) && (SC_ggi(GVAR_BOMBSTATUS__+k*2) != BOMB_STATUS_EXPLODED)) break;

				if ((k >= NUM_BOMBS) && (!scoreUpdated)) {
...
void SetRecovers(int lastTarget) {
	char	txt[32];
	int		i;


	if (lastTarget >= NUM_BOMBS-1) return; //-1 protoze posledni bomba nastavuje respawny zase na zacatek

	CLEAR(gRecs);

	gLastTarget = lastTarget;

	for (i=0;i<REC_MAX;i++){		
		sprintf(txt,REC_WPNAME_US,gLastTarget+1,i);			
	...
	if (gRecs[0]==0) SC_message("no US recover place defined!(%d)", lastTarget);

	for (i=0;i<REC_MAX;i++){		
		sprintf(txt,REC_WPNAME_VC,gLastTarget+1,i);			
	...
	if (gRecs[1]==0) SC_message("no VC recover place defined!(%d)", lastTarget);					
}
...
/*#if _GE_VERSION_ >= 138
	...
	switch(info->message){
				...
				switch(gBombStatus[i]){
					...
					case BOMB_STATUS_EXPLODED:
							...
							if (k >= NUM_BOMBS) {
								...
								for (k=0; k<NUM_BOMBS; k++)
									if ( (k>gLastTarget) && (SC_ggi(GVAR_BOMBSTATUS__+k*2) == BOMB_STATUS_EXPLODED) ) 
										SetRecovers(k);//posledni bomba, ktera bybuchla, udava cislo respawnu
							}
		...
		case SC_NET_MES_CLIENT_TICK:
			...
			SC_MP_SetSideStats(0,0,gCLN_SidePoints[COOP_SIDE_INDEX]);
			SC_MP_SetSideStats(1,0,gCLN_SidePoints[OPP_SIDE_INDEX]);
			...
			for (j=0; j<NUM_BOMBS; j++) {
				switch(gCLN_BombStatus[j]){
					...
					case BOMB_STATUS_EXPLODED:				
							...
							if (k >= NUM_BOMBS) usExploded = TRUE;
#endif
							gRecv_BombTimer[j] = gCLN_BombTimer[j] = 0.0f;
							break;
			...
			for (i=0; i<NUM_BOMBS; i++) {
				...
				//switch (gBombStatus) {
					...
					case BOMB_STATUS_EXPLODED:
						...
						if (j >= NUM_BOMBS) text_id = 1077+COOP_SIDE_INDEX; else text_id=0;
#endif
						break;
		...
		case SC_NET_MES_SERVER_KILL:
			if (abl_lists<64){
				abl_list[abl_lists] = info->param1;
				abl_lists++;
			}
