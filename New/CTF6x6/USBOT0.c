/*
 * USBOT0.c - Bot AI Script (Generated from USBOT0_reconstructed.c)
 ...
 */
void SetupUS(s_SC_P_Create *pinfo) {
    pinfo->inifile = "Ini\\Players\\NET_US_ROSENFIELD.ini";
    ...
    switch (order) {
        ...
        case 1:
            ...
            if (IsNear3DHelper(pl_id, &mycurflag, 15.0f)) {
                SC_P_Ai_SetBattleMode(pl_id, SC_P_AI_BATTLEMODE_ATTACKAGR);  // OPRAVA: Zpět na agresivní!
                myorder = 7;
                SetMoveSpeed(pl_id);
                SC_P_Ai_Go(pl_id, &encurflag);
                return 1;
            } else {
                // OPRAVA: Fallback - jdi na nepřátelskou vlajku i když nejsi blízko vlastní
                SetMoveSpeed(pl_id);
                SC_P_Ai_Go(pl_id, &encurflag);
                myorder = 2;
                return 1;
...
void MainAILoop(s_SC_P_info *info) {
    ...
    // NOVÉ: Vlajkonoš - běž přímo domů plnou rychlostí, agresivně!
    if (am_flag_carrier) {
